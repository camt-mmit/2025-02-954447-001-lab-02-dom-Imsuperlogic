...
export function createComponent(componentElem) {
    ...
    const createSectionComponent = () => {
        ...
        const recalculateResult = () => {
            ...
            ].reduce((result, elem) => result +
                (Number.isNaN(elem.valueAsNumber)
                    ? 0
                    : elem.valueAsNumber), 0);
            [...sectionContainer.querySelectorAll('.app-out-number')].forEach((elem) => (elem.textContent = result.toLocaleString()));
        };
        const createInputComponent = () => {
            // แก้จุด cloneNode (2)
            const inputContainer = numberTemplate.content.cloneNode(true).firstElementChild;
